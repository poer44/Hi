				...
				ArrayList<Video> list = (ArrayList<Video>) request.getAttribute("video");
					...
					for (int i = 0; i < Integer.parseInt(request.getAttribute("length").toString()); i++) {
						...
						out.print("<tr><td>"+ list.get(i).getVideoname()
								+ "</td><td width='80px'>"+ list.get(i).getFilesize()
								+"</td><td><img src=../../../"+ list.get(i).getImgurl()+ " width=200px;height=100px;></td><td><video controls=controls height=110px width=200px src=../../../"+ list.get(i).getFileurl() + "></td><td width='70px'>"+list.get(i).getFormat().substring(6) + "</td><td width='100px'>"
								+ time + "</td><td>"+ gettypename + 
								"</td><td width='70px'><a onclick=delvpass('"+list.get(i).getVideoid()+"')><span class='glyphicon glyphicon-remove' style='color:red;'>取消</span></a></td></tr>");
					}
	...
	$().ready(function(){
		...
		$("#pagination3").pagination({
			...
			callback: function(current) {
				window.location.href="../../"+current+"/"+$('#userid').val()+"/"+$('#userid').val()+"/submission.htm";
			}
		});
	})
	function delvpass(videoid){
		var delc=confirm('确认要取消收藏视频吗？');
		if(delc==true){
			$.ajax({
				type : "get",
				async : true,
				url : "../../"+videoid+"/"+$('#userid').val()+"/dislike.htm",
				success : function() {
					console.log(1);
